#Requires -Modules Az.Compute, Az.KeyVault, Az.Network, Common, Microsoft.PowerShell.ConsoleGuiTools

<#
.SYNOPSIS
    Copies files to a virtual machine using Bastion tunnel and scp

.DESCRIPTION
    https://github.com/equinor/ideation-machine-infra/wiki/IM-Toolbox.NOTES

.EXAMPLE
    Invoke-UploadWithBastionTunnel `
        -SourceFile `
            'C:/Dev/Discovery/Template/docker-compose.yaml', `
            'C:/Dev/Discovery/Template/docker-compose-production.yaml', `
            'C:/Dev/Discovery/Template/.env', `
            'C:/Dev/Discovery/Template/.compose.backend.production.env' `

    Invoke-UploadWithBastionTunnel -UseSshKey -SourceFile 'C:/Dev/Discovery/*'
#>

function Invoke-IMUploadWithBastionTunnel {
    [Alias('Invoke-UploadWithBastionTunnel')]
    [CmdletBinding()]
    param (
        [string]$VmName,
    ...
    Begin {
        ...
        if ($IsWindows) {
            ...
            if (!($SourceFile)) {
                $SourceFile = Open-File -multiSelect
            }
        }
        else {
            $sshFolder = "$env:HOME/.ssh"
            if (!($SourceFile)) {
                $SourceFile = Read-Host -Prompt 'Input path to at least one SourceFile'
            }
        }
    ...
    Process {
        ...
        # Create an SSH Keyfile from the Key Vault secret
        if ($UseSshKey) {
            if ($SuperSecretPw) {
            ...
            # Copy files
            scp -i "$sshFolder\sshkey.tmp" -P $LocalPort $files.FullName "$($VmUserName)@127.0.0.1:$($DestinationPath)"
        }
        else {
            if ($SuperSecretPw) {
            ...
            # Copy files
            scp -P $LocalPort $files.FullName "$($VmUserName)@127.0.0.1:$($DestinationPath)"
        }
    }
